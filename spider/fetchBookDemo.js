...
const getBookDetail = async () => {
  ...
  obj.publishDate = publishDate ? moment(publishDate).format("YYYY-MM-DD") : "";
  obj.author = author;

  // sequelize.sync({})
  try {
    // Books.sync() 和 ins.save 不可以同时进行,也不是这么说，而是该说
    // 如果要利用sync修改model结构，这件事不要同时和save同时进行
    Books.sync({ alter: true });
    // const ins = await Books.build(obj);
    // ins.save();
    console.log("success");
  } catch (e) {
    console.log("error", e);
